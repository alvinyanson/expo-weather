...
- **Notifications**: `src/hooks/useNotifications.ts` registers an Expo push token (requires the EAS `projectId` from `app.json`). `expo-notifications` is configured as a plugin.
